...
async function updateRankedBeatmaps(sinceDate = "2007-01-01") {
    try {
        ...
        for (const beatmap of beatmaps) {
            sinceDate = beatmap.approved_date
            await upsertBeatmap(beatmap);
            console.log('updated data for beatmap_id', beatmap.beatmap_id)
        }

        console.log(sinceDate)
        if (beatmaps.length == 500) {
            updateRankedBeatmaps(sinceDate);
        } else {
